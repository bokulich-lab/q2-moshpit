...
def _run_busco(output_dir: str, 
    ...
    for sample_dir in sample_dirs:
        ...
        if os.path.isfile(path_to_run_summerie):
            ...
            raise FileNotFoundError(f"BUSCO batch summary file {path_to_run_summerie} not found.")

    # Return a dict where key is sample name and value is path "tmp/sample/batch_summary.txt"
    return path_to_run_summeries

def _draw_busco_plots(path_to_run_summeries: dict,
                      output_dir: str) -> dict:
    """
...
def plot_busco(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        path_to_run_summeries = _run_busco(output_dir=results_dir, 
                                           mags=mags, 
                                           params=common_args)
        ...
        paths_to_plots = _draw_busco_plots(path_to_run_summeries=path_to_run_summeries, 
                                           output_dir=plots_dir)  

        # Zip graphs for user download
        zip_name = os.path.join(tmp, "busco_plots.zip")
        _zip_busco_plots(paths_to_plots=paths_to_plots, 
                         zip_path=zip_name)
        
        # TODO: Copy BUSCO results from tmp dir to output_dir

        # Render qiime html report
        ## Prepare context for jinja2 template
        # TODO: Figure out how exacty to make a plot with altair
        context = {
            "tabs": [
