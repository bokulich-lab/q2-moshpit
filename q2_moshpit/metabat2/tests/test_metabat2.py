...
class TestMetabat2(TestPluginBase):
    ...
    def test_get_sample_name_from_path(self):
        obs = _get_sample_name_from_path("/a/b/sampleX.fasta", ".fasta")
        exp = "sampleX"
        self.assertEqual(exp, obs)

    def test_get_sample_name_from_path_underscores(self):
        obs = _get_sample_name_from_path(
            "/a/b/sampleX_something.fasta", "_something.fasta"
        )
        exp = "sampleX"
        self.assertEqual(exp, obs)

    def test_assert_samples_ok(self):
        contigs = ["/a/b/s1_contigs.fa", "/a/b/s3_contigs.fa", "/a/b/s2_contigs.fa"]
        maps = [
            "/a/b/s3_alignment.bam",
            "/a/b/s2_alignment.bam",
            "/a/b/s1_alignment.bam",
        ]

        obs_samples = _assert_samples(contigs, maps)
        exp_samples = {
            "s1": {"contigs": "/a/b/s1_contigs.fa", "map": "/a/b/s1_alignment.bam"},
            "s2": {"contigs": "/a/b/s2_contigs.fa", "map": "/a/b/s2_alignment.bam"},
            "s3": {"contigs": "/a/b/s3_contigs.fa", "map": "/a/b/s3_alignment.bam"},
        }
    ...
    def test_assert_samples_non_matching(self):
        contigs = ["/a/b/s1_contigs.fa", "/a/b/s4_contigs.fa", "/a/b/s2_contigs.fa"]
        maps = [
            "/a/b/s3_alignment.bam",
            "/a/b/s2_alignment.bam",
            "/a/b/s1_alignment.bam",
        ...
        with self.assertRaisesRegex(
            Exception, "contigs for samples: s1,s2,s4 but maps for samples: s1,s2,s3"
        ):
            _assert_samples(contigs, maps)
