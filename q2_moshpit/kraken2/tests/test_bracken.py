...
class TestBracken(TestPluginBase):
    ...
    def test_assert_read_lens_avail(self):
        ...
        obs = _assert_read_lens_available(bracken_db=db, read_len=150)
    ...
    def test_run_bracken_one_sample(self, p1):
        ...
        p1.assert_called_once_with(
            cmd=[
                ...
                os.path.join(self.temp_dir, "sample1.bracken.output.txt"),
                "-w",
                os.path.join(bracken_report_dir, "sample1.report.txt"),
                "-t",
                str(self.kwargs["threshold"]),
                "-r",
                str(self.kwargs["read_len"]),
                "-l",
                self.kwargs["level"],
            ],
            verbose=True,
