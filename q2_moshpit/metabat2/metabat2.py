...
def _assert_samples(contigs_fps, maps_fps) -> dict:
    ...
    contig_samps = [_get_sample_name_from_path(x, "_contigs.fa") for x in contigs_fps]
    map_samps = [_get_sample_name_from_path(x, "_alignment.bam") for x in maps_fps]
    if set(contig_samps) != set(map_samps):
        raise Exception(
            "Contigs and alignment maps should belong to the "
