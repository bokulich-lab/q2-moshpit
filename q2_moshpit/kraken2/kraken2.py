...
def _classify_kraken(manifest, common_args) -> (
    ...
    try:
        for index, row in manifest.iterrows():
            ...
            sample_dir_report = os.path.join(
                kraken2_reports_dir.path, _sample
            )
            sample_dir_output = os.path.join(
                kraken2_outputs_dir.path, _sample
            )
            for s in [sample_dir_report, sample_dir_output]:
                os.makedirs(s, exist_ok=True)
            report_fp = os.path.join(
                sample_dir_report, f'{_bin}.report.txt'
            )
            output_fp = os.path.join(
                sample_dir_output, f'{_bin}.output.txt'
            )
            cmd.extend([
                '--report', report_fp, '--output', output_fp, '--use-names',
                *fn
            ])
